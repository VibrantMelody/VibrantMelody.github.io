...
function createGhost() {
  ...
  const ghostBox = document.querySelector(
    `#main-container > :nth-child(${randomNumGenerator(1, horizontalBoxes * verticalBoxes)})`,
  );
...
function displayGrid() {
  ...
  horizontalBoxes = Math.ceil(
    mainContainer.getBoundingClientRect().width / 76.4,
  );
  verticalBoxes = Math.ceil(
    mainContainer.getBoundingClientRect().height / 76.4,
  );
  ...
  for (let i = 1; i <= horizontalBoxes * verticalBoxes; i++) {
    ...
    childElement.setAttribute(
      "style",
      `width: ${100 / horizontalBoxes}%; height: ${100 / verticalBoxes}%;`,
    );
  ...
  const middleBox = document.querySelector(
    `#main-container > :nth-child(${Math.round((horizontalBoxes * verticalBoxes) / 2 - horizontalBoxes / 2)})`,
  );
...
function move(direction) {
  ...
  switch (direction) {
    ...
    case "up":
      ...
      parentBox = document.querySelector(
        `.${`box${parseInt(currentParentClass[0]) - horizontalBoxes}`}`,
      );
      difference = currentParentClass[0] - horizontalBoxes;
      difference <= 0 &&
        (parentBox = document.querySelector(
          `.${`box${difference + horizontalBoxes * verticalBoxes}`}`,
        ));
      break;
    case "down":
      currentParentClass = pacman.parentElement.classList[1].match(/\d+/g);
      ...
      difference > horizontalBoxes * verticalBoxes &&
        (parentBox = document.querySelector(
          `.${`box${difference - horizontalBoxes * verticalBoxes}`}`,
        ));
      break;
